...
class Customers(CustomerHandler):
	...
	def post(self, *args, **kwargs):
		...
		new_customer.firstName = self.request.get('firstname')
		new_customer.lastName = self.request.get('lastname')
		new_customer.email = str(self.request.get('email')).lower()
	...
	def put(self, *args, **kwargs):
		...
		if check: 
			...
			if re.match(db_defs.Customer.emailRegex(), self.request.get('email')):
				# check sku does not already exist
				q = db_defs.Customer.query()
				q = q.filter(db_defs.Customer.email == str(self.request.get('email')).lower())
			...
			if self.request.get('firstname'):
				check.firstName = self.request.get('firstname')

			if self.request.get('lastname'):
				check.lastName = self.request.get('lastname')

			if self.request.get('email'):
				check.email = str(self.request.get('email')).lower()
...
class CustomerSearch(CustomerHandler):
	...
	def get(self, *args):
		...
		if self.request.get('email'):
			q = q.filter(db_defs.Customer.email == str(self.request.get('email')).lower())
		if self.request.get('lastname'):
			q = q.filter(db_defs.Customer.lastName == self.request.get('lastname'))
		if self.request.get('firstname'):
			q = q.filter(db_defs.Customer.firstName == self.request.get('firstname'))
